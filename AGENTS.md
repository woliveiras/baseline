...
- Run `pnpm run eval:full` only with explicit maintainer authority because it invokes model calls and consumes quota. It runs the official validators, deterministic checks, config validation, then all routing, behavior, and frozen security cases before proving Git status is unchanged. Routing and behavior may use disjoint shards with at most two active processes; do not reduce coverage or reasoning to optimize elapsed time. It is an explicit empirical evaluation, not a pre-push hook or an automatic Git gate.
