...
Install the complete Tuxedo plugin from a trusted checkout without copying repository-only files or development dependencies into the Codex plugin cache. Preserve the root compatibility path for repository tooling while directing public standalone installation to the canonical packaged skill tree.
...
- Local plugin installation must not copy `node_modules/`, `evals/`, `specs/`, tests, project documentation, repository configuration, or authentication state.
...
- **CP-002** The package top level contains only `.codex-plugin/` and `skills/`; it contains no symlink escaping the package and no repository-only or runtime dependency path.
- **CP-003** The root `skills/` path resolves to `plugins/tuxedo/skills`, so existing repository tooling uses the same canonical files while public standalone instructions name the canonical path directly.
...
- **CP-007** README and project documentation explain the package boundary, direct CLI installation, standalone installation, updates, removal, and the absence of a package-build step.
...
- Symlinks are acceptable for the root compatibility path on the supported macOS and Linux development environments. The installed package itself contains real files.
