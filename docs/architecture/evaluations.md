...
| `EV-RTE-01` | Routing cases measure explicit invocation of an applicable skill and explicit non-reading of an inapplicable skill, rather than relying on nondeterministic spontaneous discovery. A negative case requires an alternate skill only when that alternate skill's trigger contract applies. | Generated requests name each expected `SKILL.md` and forbid opening the avoided `SKILL.md`; `negative-refine` forbids `refine` without inventing an alternate expectation. Assertions still require structured provider metadata for both signals. | spec-derived |
