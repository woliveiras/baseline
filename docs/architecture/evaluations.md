...
| `EV-RTE-01` | Routing cases measure explicit invocation of an applicable skill and rejection of an inapplicable skill, rather than relying on nondeterministic spontaneous discovery. A negative case requires an alternate skill only when that alternate skill's trigger contract applies. | Every expected skill request names its assigned `SKILL.md`; `negative-refine` forbids `refine` without inventing an alternate expectation. Assertions still require structured provider metadata and preserve the forbidden-skill check. | spec-derived |
