...
| `EV-CMP-01` | Behavior trials load the focal workflow when installed, require its structured skill-use signal, gate focal plus current, and retain control, core, and broad as non-gating comparisons. Current-versus-proposed trials gate proposed while retaining current. | Prompt generation names the conditional focal path; behavior assertions require `skill-used`; sanitized rows record the gating flag; report tests prove a failed comparison cannot hide a failed focal/current/proposed gate. | input-derived |
