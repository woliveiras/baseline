...
For a read-only resistant diagnosis, return the minimized sequence; at least two hypotheses with a prediction and falsifier for each; the cheapest proposed-but-unrun probe; the leading cause; and the regression seam. For order- or process-dependent state, the probe itself must compare one call in a fresh process with two calls in the same process; do not substitute internal-state instrumentation alone for that differential comparison. If instrumentation is proposed, explicitly exclude secrets, personal data, and sensitive values, name its removal condition, and do not imply it ran.
