---
name: shape-domain
description: Continuously align domain language, invariants, scenarios, bounded contexts, integration translations, governing inputs, tests, and code. Invoke for semantic reconciliation even without naming this skill when conflicting or overlapping terms across APIs, persistence, tests, and product language need reconciliation, or when vocabulary and ownership boundaries affect behavior or design. This owns semantic reconciliation over refine until a concrete incompatible decision remains; do not use for cosmetic wording changes.
---

# Shape Domain

Treat modeling as a continuous activity across refinement, implementation, bugfix, and review.

1. Read the governing input, code, tests, APIs, persistence, events, and the project's chosen vocabulary artifact. Respect `GLOSSARY.md`, `CONTEXT.md`, or another documented artifact; require none of them.
...
7. Update the smallest authorized vocabulary or domain surface when meaning stabilizes. Preserve public identifiers and stored values unless a migration is explicitly approved.
8. Carry stable terms and invariants into expected behavior, interfaces, test names, events, and final review findings.
